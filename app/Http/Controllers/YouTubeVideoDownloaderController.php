...
class YouTubeVideoDownloaderController extends Controller
{
        ...
        public function getPlaylistVideos($id)
        {
            ...
            $videos = $this->curl_get_contents('https://www.googleapis.com/youtube/v3/playlistItems?'.http_build_query(['part' => 'snippet', 'maxResults' => 50, 'playlistId' => $playlist_id, 'key' => $this->google_api_key]));
            $videos = json_decode($videos, true);
            $result = [];
            if (!empty($videos['items'])) {
                foreach ($videos['items'] as $video) {
                    $result[] = ['title' => $video['snippet']['title'], 'video_id' => $video['snippet']['resourceId']['videoId']];
                }
